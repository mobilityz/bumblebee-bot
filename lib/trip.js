...
const polylineLib = require('polyline');
 ...
 */
const generate_trip = function(bot, origin, callback) {
...
const generate_directions = function(coords, callback) {
  googleMapsClient.directions(coords, function(err, response) {
    if (err) {
      console.error("googleMapsClient error:" + err);
...
const generate_road = function(directions, callback) {
