 ...
 */
var generate_trip = (bot, origin, callback) => {
    ...
    trip.generate_directions(coords, (directions) => {
      trip.generate_road(directions, callback);
    });
  };

var generate_directions = (coords, callback) => {
    googleMapsClient.directions(coords, (err, response) => {
      if (err) {
        console.log(err)
...
var generate_road = (directions, callback) => {
    googleMapsClient.snapToRoads({path: directions, interpolate: true}, (err, response) => {
      if (err) {
        console.log(err)
        return false; // Should also return err
      } else {
